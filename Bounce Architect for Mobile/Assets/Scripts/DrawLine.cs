...
public class DrawLine : MonoBehaviour
{
    ...
    public float inkLimit, inkMultiplier;
    public Text inkText;
    ...
    void Update()
    {
        if (isDraw) {
            ...
            if (Input.GetMouseButton(0) && drawStarted) {
                Vector2 tempDrawPos = Camera.main.ScreenToWorldPoint(Input.mousePosition);
                if (tempDrawPos.x > drawTopLeft.position.x && tempDrawPos.x < drawBottomRight.position.x &&
                    tempDrawPos.y < drawTopLeft.position.y && tempDrawPos.y > drawBottomRight.position.y) {
    ...
    void UpdateLineNewPoint(Vector2 newDrawPos) {
        ...
        inkUsed += Vector2.Distance(drawPositions[drawPositions.Count - 1], newDrawPos) * inkMultiplier;
        if (inkUsed > inkLimit) {
            if (!isSpinning)
                StartCoroutine(LineFinish());
        } else {
    ...
    [HideInInspector]public bool rightPress, leftPress;

    void MoveLine() {        
        if ((Input.GetAxisRaw("Horizontal") > 0 || rightPress) && pivot.transform.position.x < xBounds) {
        ...
        if ((Input.GetAxisRaw("Horizontal") < 0 || leftPress) && pivot.transform.position.x > -xBounds) {
