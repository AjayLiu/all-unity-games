...
public class rainbowColorScript : MonoBehaviour {
    ...
    void Update() {
        if(isOn) {
		    if(isImg) {
			    img.material.SetColor("_Color", HSBColor.ToColor(new HSBColor(Mathf.PingPong(Time.time * Speed, 1),1,1)));
		    } else if(isText) {
			    t.color = HSBColor.ToColor(new HSBColor(Mathf.PingPong(Time.time * Speed, 1),1,1));
		    }else if(isRend)
			    s.color = HSBColor.ToColor(new HSBColor(Mathf.PingPong(Time.time * Speed, 1), 1, 1));
        }
    }
}
